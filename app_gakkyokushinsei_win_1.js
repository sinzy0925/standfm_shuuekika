...
async function gakkyokushinsei2(headlessMode,testmode,gakuArr,j,res_gakkyoku,res_gakkyoku2,resArr,JorN,standfm_id){
  let startTime = performance.now(); // 開始時間
  let endTime = performance.now(); // 終了時間
  ...
  const standfm_listNo  = String(gakuArr[j][1]);
  let code_title = ''
  for(let i = 0 ; i < res_gakkyoku2.length ;i++){
    code_title += 'APP楽曲申請済み [' + res_gakkyoku2[i].split(',')[0] + '🌸' +res_gakkyoku2[i].split(',')[1] + ']\n'
  }
console.log(['code_title',code_title])
...
*/
  ...
  try{
    ...
    await page2.getByPlaceholder('放送の概要・ハッシュタグ・補足情報など').press('Home');
    await page2.getByPlaceholder('放送の概要・ハッシュタグ・補足情報など').press(' ');
    await page2.getByPlaceholder('放送の概要・ハッシュタグ・補足情報など').press('Control+a');
    await page2.getByPlaceholder('放送の概要・ハッシュタグ・補足情報など').press('Control+c');
    await page2.getByPlaceholder('放送の概要・ハッシュタグ・補足情報など').fill(code_title);
    await page2.getByPlaceholder('放送の概要・ハッシュタグ・補足情報など').press('Control+v');
    await page2.getByText('放送の説明').click();
    await page2.locator('body').press('ArrowUp');
    await page2.locator('body').press('ArrowUp');
    await page2.locator('body').press('ArrowUp');
    ...
    if(titeled == -1 && titlename.length <= 40 - gakkyokushinseisumi.length ){
      ...
      await page2.getByPlaceholder('タイトルを入力').press('Control+a');//Ctrl + a
      await page2.getByPlaceholder('タイトルを入力').press('Control+c');//Ctrl + c
      await page2.getByPlaceholder('タイトルを入力').fill(gakkyokushinseisumi);
      await page2.getByPlaceholder('タイトルを入力').press('ArrowUp');
      await page2.getByPlaceholder('タイトルを入力').press('Control+v');//Ctrl + V
      await page2.waitForTimeout(2000);  
      console.log('     アーカイブのタイトル変更します！！！ ' + gakkyokushinseisumi)
      //console.log(titlename)
    }else{
